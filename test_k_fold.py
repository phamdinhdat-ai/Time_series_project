...
folder = 'data/new_data_static/all_person/'
K = opt.k_fold
...
for fold_idx, (train_idx, val_idx) in enumerate(kf.split(list_paths)):
    print("Fold Id: ", fold_idx)
    print("Train Folder:  ", train_idx)
    print("Val Folder:  ", val_idx)
    
    k_folds_dir_i = f'{k_folds_dir}/K_fold_{fold_idx}'
    os.makedirs(k_folds_dir_i, exist_ok=True)
    train_folder = f'{k_folds_dir_i}/trainset'
    test_folder  = f'{k_folds_dir_i}/testset'
    os.makedirs(train_folder, exist_ok=True)
    os.makedirs(test_folder, exist_ok=True)    
    for id in train_idx:
        source_path = f'{folder}/{list_paths[id]}'
        destination_path = f'{train_folder}/{list_paths[id]}'
        ...
        # print(f"Contents of '{folder}' have been copied to '{train_folder}'.")
        
        
    for id in val_idx: 
        source_path = f'{folder}/{list_paths[id]}'
        destination_path = f'{test_folder}/{list_paths[id]}'
        if os.path.isdir(source_path):
            # Recursively copy subdirectories
            shutil.copytree(source_path, destination_path, dirs_exist_ok=True)
        else:
        ...
        # print(f"Contents of '{folder}' have been copied to '{test_folder}'.")
    ...
    print("=" * 50)
    print(f"Fold {fold_idx}:Training Done...")
    print("=" * 50)

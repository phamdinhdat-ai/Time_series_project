...
#     val_X, sequence_labels_val = generate_data(val_scaled, val_label, sequence_length= sequence_length, step= steps)
...
def main_k_fold(opt):
    ...
    if opt.data_type == 'k_fold':
        count = 1
        k_fold_results = []
        k_fold_hist = []
        for folder in os.listdir(k_fold_static_path):
            print(f"Trainin {model_type} on {count}-fold static dataset: \n ")
            trainset = np.load(f'{k_fold_static_path}/{folder}/static_train_set.npy')
            testset = np.load(f'{k_fold_static_path}/{folder}/static_test_set.npy')
